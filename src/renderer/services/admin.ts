...
import { SystemInfo, BackupPreviewInfo } from '../types';
...
export async function restoreBackup(filePath: string): Promise<{ message: string; restoredCounts: Record<string, number>; backupDate: string }> {
  return unwrap(await window.electronAPI.admin.restore(filePath));
}

export async function previewBackupFile(filePath: string): Promise<BackupPreviewInfo> {
  return unwrap(await window.electronAPI.admin.previewBackup(filePath));
}
